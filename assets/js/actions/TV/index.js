...
export const fetchGenreTv = () => dispatch => {
	axios
		.get(`https://api.themoviedb.org/3/genre/tv/list?${API_KEY}`)
		.then(res =>
			dispatch({
				type: FETCH_GENRE_TV,
				payload: res.data
...
export const fetchPopularTv = () => dispatch => {
	axios
		.get(`https://api.themoviedb.org/3/tv/popular?${API_KEY}`)
		.then(res =>
			dispatch({
...
export const fetchLatestTv = () => dispatch => {
	axios
		.get(`https://api.themoviedb.org/3/tv/latest?${API_KEY}`)
		.then(res =>
			dispatch({
...
export const fetchTopRatedTv = () => dispatch => {
	axios
		.get(`https://api.themoviedb.org/3/tv/top_rated?${API_KEY}`)
		.then(res =>
			dispatch({
...
export const fetchTvCredits = id => dispatch => {
	axios.get(`https://api.themoviedb.org/3/tv/${id}/credits?${API_KEY}`).then(res =>
		dispatch({
